    ...
    getSelectionStartFromPointer: function(e) {
        ...
        for (var i = 0, len = textLines.length; i < len; i++) {
            ...
            if (mouseOffset.y < height) {
                ...
                return this._getNewSelectionStartFromOffset(mouseOffset, prevWidth, width, charIndex - this.lineEndCursorOffset, jlen);
            }
        }
    ...
    fabric.Textbox = fabric.util.createClass(fabric.IText, fabric.Observable, {
        ...
        initializeTextboxBehavior: function() {
            ...
            fabric.Canvas.prototype._setObjectScale = function(localMouse, transform, lockScalingX, lockScalingY, by, lockScalingFlip) {
                ...
                if (typeof t.scaleBehavior !== "undefined" && t.scaleBehavior === "textbox_resize") {
                    var w = t.width * (localMouse.x / transform.scaleX / (t.width + t.strokeWidth)), h = t.height * (localMouse.y / transform.scaleY / (t.height + t.strokeWidth));
                    var resizeHeight = [ "mb" ].indexOf(t.__corner) !== -1, resizeWidth = [ "mr" ].indexOf(t.__corner) !== -1, resizeBoth = [ "br" ].indexOf(t.__corner) !== -1, sw = t.currentWidth, sh = t.currentHeight;
        ...
        _wrapText: function(ctx, text, returnHeightMode) {
            ...
            for (i = 0; i < lines.length; i++) {
                var limit = returnHeightMode ? Infinity : this.currentHeight - totalHeight, wrappedLine = this._wrapLine(ctx, lines[i] + "\n", limit);
        ...
        _renderViaNative: function(ctx) {
            ...
            gctx = ctx;
            if (this.autoresize) {
                var maxHeight = this.currentHeight, minFontPixels = this.minFontPixels, maxFontPixels = this.maxFontPixels <= 0 ? maxHeight : this.maxFontPixels;
                var oldFontSize = this.fontSize, autoFontSize = this._autosizing(ctx, maxHeight, minFontPixels, maxFontPixels);
